...
import { ArrowUpRight, Copy, Check, Terminal, CheckCheck } from "lucide-react";
import { cn } from "@/lib/utils";
import { AnimatePresence, easeOut, motion } from "motion/react";
import { copyComponent } from "@/lib/action";
import { OpenInV0Button } from "../ui/open-in-v0";
...
const PreviewClient: FC<PreviewContent> = ({
  isBlock = false,
  link,
  prePath,
}) => {
  ...
  const [isTerminalCopied, setIsTerminalCopied] = useState<boolean>(false);
  ...
  const handleTerminalClick = () => {
    const [folder, fileName] = link?.split("/") ?? [];
    const COPY = `pnpm dlx shadcn@latest add ${prePath}/r/${
